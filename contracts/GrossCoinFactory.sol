...
contract GrossCoinFactory {
    ...
    mapping(address => bool) public isGrossCoin; //verify without having to do a bytecode check.
    bytes public GrossCoinByteCode; // solhint-disable-line var-name-mixedcase
    ...
    function verifyGrossCoin(address _tokenContract) public view returns (bool) {
        bytes memory fetchedTokenByteCode = codeAt(_tokenContract);

        if (fetchedTokenByteCode.length != GrossCoinByteCode.length) {
            return false; //clear mismatch
        }

      //starting iterating through it if lengths match
        for (uint i = 0; i < fetchedTokenByteCode.length; i++) {
            if (fetchedTokenByteCode[i] != GrossCoinByteCode[i]) {
                return false;
            }
        }
        return true;
