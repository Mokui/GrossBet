    ...
    if (j == maxFrames) {
        anim2 = function () {
            if (player2.dead == false) {
                ...
                } else if (keys[40] || b_P2 == true) { //attack with bottom arrow
                    socket.emit("action", 40);
                    // attack
                    if (player2.lastDir == "l") {
...
function update() {
    // jump
    // player 1
    if (keys[90] || a_P1 == true) {
        socket.emit("action", 90);
        if (!player1.jumping) {
